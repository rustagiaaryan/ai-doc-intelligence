// FILE: services/web-frontend/src/pages/ChatWithHistory.tsx
...
import { Document, ChatMessage, DocumentChunk, Conversation, ConversationWithMessages } from '../types';
...
const ChatWithHistory: React.FC = () => {
  ...
  const loadConversation = async (conversationId: string) => {
    try {
      const conversation: ConversationWithMessages = await conversationsApi.getConversation(conversationId);

      const chatMessages: ChatMessage[] = conversation.messages.map(msg => ({
        role: msg.role,
        content: msg.content,
        timestamp: new Date(msg.created_at),
        source_chunks: msg.source_chunks ? JSON.parse(msg.source_chunks) : undefined,
      }));
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const assistantMessage: ChatMessage = {
        ...
        timestamp: new Date(),
        source_chunks: response.retrieved_chunks,
      };
